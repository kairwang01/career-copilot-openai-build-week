...
| Test-driven hardening | Used failing tests and CI evidence to narrow root causes, review targeted fixes, and preserve exact-commit evidence instead of treating plausible code as proof. | [`.github/workflows/ci.yml`](.github/workflows/ci.yml), [`scripts/run-release-gate.mjs`](scripts/run-release-gate.mjs), [`tests/`](tests/) |
